...
An ADR records one architectural decision or cross-cutting standard, with its context, the decision, and its consequences. It answers "how do we always do X here": how the two content-script worlds talk, what the extension stores where, which files carry tests, and why there is no build step.
...
`NNNN-short-descriptive-title.md`, sequential, never renumbered. Gaps are fine, and this folder already has two: `0002` and `0005` stayed in `google-photos-compare-and-save` when this repository was split out of it. Use `TEMPLATE.md` as the starting point: Context, Decision, Consequences.
...
- A new setting or a new timing default. Those are code and belong in `AGENTS.md` rules at most.
