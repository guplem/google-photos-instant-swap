...
- **The two content-script worlds** - `manifest.json` content-script entries, `mainWorldMotionPatch.js`, `isolatedWorldBootstrap.js`, `publishSettings`, and any new `data-gpis-*` attribute or new channel between the worlds.
- **Extension storage** - `extensionSettings.js`, any new storage key, any change to which `chrome.storage` area holds what, and any record-shape change that `normalizeSettings` cannot repair.
- **How the extension shortens motion** - the CSS rules in `instantSwapStyles.css`, the `matchMedia` and `Element.prototype.animate` patches, and anything that would make the extension read or click the page instead.
- **The testing strategy** - the list of files exempt from unit tests, or any move toward a browser-driven test.
...
**Not an ADR:** a new setting, a timing default, standard library usage, one-off bug fixes, anything a reader can derive from the code or from `README.md`, minor refactors.
