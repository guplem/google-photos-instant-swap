...
2. **For bugs, verify the bug actually exists in the current code.** If the code looks correct, report: "I checked the code and this appears to already be handled in `<file>:<line>`. Are you sure you want to create this issue?" Stop if the user agrees it is not a bug.
   - One caution specific to this repo: a bug report about a photo that still slides is often **not** a code bug. Check first whether the reporter turned on the stronger settings, in the order the `README.md` Troubleshooting section gives, and whether they reloaded the Google Photos tab after each change.
...
- Good (change): "Today the stronger level shortens every keyframe animation on the page; this issue leaves the loading spinners alone."
- Good (new capability): "Add a keyboard shortcut that turns the swap off, so a user can watch one slide without opening the options page."
- Bad (after only): "Leave the spinners alone." The reader cannot tell what is wrong today.
- Bad (before only): "The stronger level also stops the spinners." States the gap but not what this issue makes true.
...
3. **Acceptance Criteria must describe observable behaviors, not implementation details.** AC answers "what does the system do?", not "what code do I write?". Good: "a menu keeps its own animation while the photo swap is instant". Bad: "add a `revert` rule to `instantSwapStyles.css`".
...
6. **Steps to Reproduce must add value.** If reproduction is trivially "open a photo and press the arrow key", omit Steps and describe the condition in Context.
