...
2. Add the default to `DEFAULT_SETTINGS`, and add the read to the object `normalizeSettings` returns. Use the existing `readBoolean` helper, or add one of the same shape for a new value type.
...
Every setting in this extension reaches the page through `publishSettings` in `src/contentEntry.js`, as a `data-gpis-*` attribute, so add it there and to the attribute table in `AGENTS.md`. Nothing else in the isolated world reads the settings.

A setting the MAIN world reads needs a page reload to take effect. Say so in its `.hint` text.
