...
Record: which areas the PR touches (`src/instantSwap/`, `src/settings/`, `src/bootstrap/`, `options/`, `test/`, `adr/`, `.github/`, `manifest.json`), the total scope (additions/deletions, file count), the stated intent (commits + PR description), and any implementation proposal from the issue discussion.
...
- **pattern-scout** (only when the diff adds a new module, setting, or motion rule): find how similar ones are built here and report the convention with real examples.
...
- **Findings index**: a numbered list of every finding that survived phase 4, so the user sees the total before the questions begin. Example:
  ```
  1. [Required]   A 0s duration cancels the transitionend event - src/instantSwap/instantSwapStyles.css:21 - fix in this PR
  2. [Suggestion] Missing test for an unknown swap level - test/extensionSettings.test.js - fix in this PR
