...
- **Storage keys are data, not code.** `settings:v1` names a record that already exists in a user's browser. Renaming the JavaScript constant is safe. Renaming the **stored string** resets every user's settings with no error. If the record shape must change, add a `:v2` key; never rename a live key.
- **A name that lives in both JavaScript and CSS.** Every `data-gpis-*` attribute is written in one place and read in another: `publishSettings` in `src/contentEntry.js`, a selector in `instantSwapStyles.css`, the `SETTINGS_ATTRIBUTE` constant in `mainWorldMotionPatch.js`, and a row in the `AGENTS.md` attribute table. Grep the plain string across `src/`, `options/`, and `AGENTS.md`, not just the JavaScript.
...
After a rename, run `npm run check`, then load the extension in Chrome once and press the arrow key on a photo. The type check cannot see a broken manifest path or a broken CSS selector.
